...
```{bash config_name_email, eval=FALSE}
...
knitr::opts_knit$set(root.dir = "git-demo")
```
```{bash ls}
ls -la
```
...
```
```{r store_commit0, echo=FALSE}
hash0 = system2("git", "rev-parse --short=7 HEAD", stdout = TRUE)
```
...
``r paste("git", paste("checkout", hash0, "test.txt"))``.
...
you can run
```{bash create_tag, eval=FALSE}
git tag -a v1.1.0 -m "version 1.1.0"
```
and to tag any commit
```{bash tag_commit, eval=FALSE}
...
system2(command = "cd", args = root)
knitr::opts_knit$set(root.dir = root)
```

```{bash del_demo, echo=FALSE}
rm -rf git-demo
```
